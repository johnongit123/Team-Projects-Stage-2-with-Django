...
$('#edit-tasks').on("click", () => {
    ...
    if ($('#edit-tasks').is(':checked')) {
        ...
        $('#to-do-list li').find('h3, p').attr('contenteditable', true);
    }
    else {
        ...
        $('#to-do-list li').find('h3, p').attr('contenteditable', false);
    }
...
to_do_list.on("click", "li .top-section input[type='checkbox']", (event) => {
    ...
    if (checkbox_clicked.is(':checked'))
        checkbox_clicked.closest('li').addClass('complete')
    else
...
$("#new-task-form").on("submit", (event) => {
    ...
    <li>
        <div class="top-section" style="background-color: ${titleColor.val()};">
            <progress value="10" max="100" class="color-1"></progress>
            <h3>${taskTitle.val()}</h3>
